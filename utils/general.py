...
# 检查图像尺寸是否是s的整倍数（默认32），不是则调整图像尺寸(本质是通过make_divisible实现的）
...
# 这个函数是检查本都相关文件路径能否找到这个文件，没找到就说明文件丢失了，返回空；
# 如果传入的是一个网络地址就直接下载这个文件；否则找到就返回本地匹配到的第一个文件名。这个函数很有用，用的很广。
...
def non_max_suppression(prediction,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference 遍历所有框
        ...
        i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS    返回值：keep：NMS过滤后的bounding box索引（降序）
...
def print_mutation(results, hyp, save_dir, bucket, prefix=colorstr('evolve: ')):
    ...
    if bucket:
        os.system(f'gsutil cp {evolve_csv} {evolve_yaml} gs://{bucket}')  # images
