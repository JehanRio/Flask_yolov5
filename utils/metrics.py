...
def box_iou(box1, box2, eps=1e-7):
    ...
    inter = (torch.min(a2, b2) - torch.max(a1, b1)).clamp(0).prod(2)    # clamp:限制上下限 prod:返回输入张量给定维度上每行的积

    # IoU = inter / (area1 + area2 - inter)
    return inter / (box_area(box1.T)[:, None] + box_area(box2.T) - inter + eps) # [:,None]：增加了一个维度
