...
def select_device(device='', batch_size=0, newline=True):
    ...
    if not (cpu or mps) and torch.cuda.is_available():  # prefer GPU if available
        ...
        # 检查是否有gpu设备 且 batch_size是否可以能被显卡数目整除
...
def model_info(model, verbose=False, img_size=640):
    ...
    n_g = sum(x.numel() for x in model.parameters() if x.requires_grad)  # number gradients 模型model的参数中需要求梯度(requires_grad=True)的参数量
    if verbose:
    ...
    try:  # FLOPs
        ...
        stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 32   # hasattr() 函数用于判断对象是否包含对应的属性
...
def scale_img(img, ratio=1.0, same_shape=False, gs=32):  # img(16,3,256,416)
    ...
    img = F.interpolate(img, size=s, mode='bilinear', align_corners=False)  # resize 插值
    if not same_shape:  # pad/crop img
        h, w = (math.ceil(x * ratio / gs) * gs for x in (h, w)) # 向上取整
        ...
        with torch.no_grad():   # 不做梯度更新
            ...
            for k, v in self.ema.state_dict().items():
                if v.dtype.is_floating_point:
                    ...
                    # 将某个node变成不需要梯度的Variable。因此当反向传播经过这个node时，梯度不会从这个node往前面传播

    def update_attr(self, model, include=(), exclude=('process_group', 'reducer')):
        # Update EMA attributes
        copy_attr(self.ema, model, include, exclude)    # 排除掉'process_group', 'reducer'
