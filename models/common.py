...
# 这个模块可以根据输入的卷积核计算该卷积模块所需的pad值。将会用于下面会讲到的Conv函数和Classify函数中。
# 为same卷积或same池化自动扩充
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)   从最后一个维度 w,h开始
        return self.conv(torch.cat((x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]), 1))
        # return self.conv(self.contract(x))


# 幻象模块虽然不能增加mAP，但是可以大大减少模型计算量。
# 使用：可以用GhostConv代替一般的Conv，GhostBottleneck代替C3，至于在哪些位置代替，可以自己决定。
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        super().__init__()
        w = str(weights[0] if isinstance(weights, list) else weights)   # isinstance(),判断路径；[0]路径
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs = self.model_type(w)  # get backend,使用pytorch，判断是什么格式的权重文件
        w = attempt_download(w)  # download if not local
        fp16 &= (pt or jit or onnx or engine) and device.type != 'cpu'  # FP16
        stride, names = 32, [f'class{i}' for i in range(1000)]  # assign defaults   步长32，names默认能检测出1000类
        if data:  # assign class names (optional)
            with open(data, errors='ignore') as f:
                names = yaml.safe_load(f)['names']  # names变成了80个类别的列表
